...
- **3 AI модели**: Juggernaut Pro Flux, Seedream V3, HiDream-I1 Fast
...
- **3 AI модели**: Juggernaut Pro Flux, Seedream V3, HiDream-I1 Fast
- **AI Upscaling**: Автоматическое улучшение качества изображений через Replicate Real-ESRGAN
- **Автоматические фоллбеки** при сбоях провайдеров
- **Динамическое переключение** моделей без перезапуска системы
- **Масштабирование изображений**: Увеличение разрешения в 4 раза с улучшением качества
...
1. **Регистрация на Replicate:**
   - Перейдите на https://replicate.com/
   - Создайте аккаунт через GitHub, Google или email

2. **Получение API токена:**
   - Войдите в личный кабинет
   - Перейдите в раздел "Account" → "API tokens": https://replicate.com/account/api-tokens
   - Нажмите "Create token"
   - Введите название токена (например, "AI Stock Bot Upscaling")
   - Скопируйте и сохраните токен (начинается с `r8_`)

3. **Настройка биллинга:**
   - Перейдите в "Billing": https://replicate.com/account/billing
   - Добавьте способ оплаты (кредитная карта)
   - Replicate работает по модели "pay-per-use" - вы платите только за фактическое использование
   - Рекомендуется установить лимиты расходов для контроля бюджета
...
4. **Проверка токена:**
   ...
   curl -s -X POST \
     ...
     -d '{"version": "42fed1c4974146d4d2414e2be2c5277c7fcf05fcc3a73abf41610695738c1d7b", "input": {"image": "https://example.com/test.jpg"}}' \
...
SEGMIND_API_KEY=your_segmind_api_key_here  # API ключ Segmind для AI моделей (нужно подставить ключ, который выдал Segmind)
REPLICATE_API_TOKEN=your_replicate_api_token_here  # API токен Replicate для AI upscaling (нужно подставить токен, который выдал Replicate, начинается с r8_)
REPLICATE_TIMEOUT=300000                   # Таймаут для Replicate API (5 минут)
REPLICATE_MODEL=nightmareai/real-esrgan:42fed1c4974146d4d2414e2be2c5277c7fcf05fcc3a73abf41610695738c1d7b  # Модель Real-ESRGAN для upscaling (не менять)
REPLICATE_SCALE=4                          # Коэффициент масштабирования изображений (4x увеличение)
REPLICATE_FACE_ENHANCE=true                # Включить улучшение лиц при upscaling (true/false)
...
curl -X POST http://localhost:3000/api/admin/config/ai-model/switch \
  -H "Content-Type: application/json" \
  -d '{"model": "juggernaut-pro-flux", "reason": "Переключение для тестирования"}'
