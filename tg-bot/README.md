...
const imageActionsKeyboard = {
  inline_keyboard: [
    [{ text: '📤 Загрузить на 123RF', callback_data: JSON.stringify({
      action: 'upload_123rf',
      imageId: imageId
    })}],
    [{ text: '🔄 Сгенерировать еще', callback_data: JSON.stringify({
      action: 'generate_more'
    })}],
    [{ text: '💰 Купить изображения', callback_data: JSON.stringify({
      action: 'buy_images'
    })}]
  ]
};
