...
## 🎛️ Управление функциональностью

### Флаг STOCKS_ENABLED

**Функциональность отключения стоковых сервисов** - это системный флаг, который позволяет полностью отключить интеграцию со стоковыми площадками в боте.

#### Ключевые особенности
- **Глобальное отключение** всех функций, связанных со стоками
- **Динамическая проверка** статуса при каждом запросе
- **Graceful degradation** - бот продолжает работать без стоковых функций
- **Автоматическое скрытие** соответствующих кнопок и команд

#### Поведение при STOCKS_ENABLED=false

##### Команды и меню
```javascript
// Команда /mystocks становится недоступной
if (!stocksEnabled) {
  return bot.sendMessage(chatId, '⚠️ *Функция стоков временно отключена*');
}

// Кнопка "⚙️ Мои стоки" скрывается из главного меню
if (stocksEnabled) {
  menuKeyboard.inline_keyboard[1].push({ 
    text: "⚙️ Мои стоки", 
    callback_data: "menu_mystocks" 
  });
}
```
...
```javascript
// Кнопки загрузки на стоки не отображаются
function getUploadOnlyKeyboard(imageId, availableServices = []) {
  const keyboard = [];
  
  // Проверка флага перед добавлением кнопок стоков
  if (stocksEnabled && availableServices.includes('123rf')) {
    keyboard.push([{ 
      text: "📤 Загрузить на 123RF", 
      callback_data: `upload_123rf_${imageId}` 
    }]);
  }
...
```javascript
// Информация о стоках исключается из /help
if (stocksEnabled) {
  helpMessage += `
📤 *Загрузка на стоки:*
...
##### Проверка статуса флага
```javascript
// Backend API endpoint для проверки флагов функций
async function isStocksEnabled() {
  try {
...
```javascript
// Проверка перед каждым действием со стоками
const stocksEnabled = await backendApi.isStocksEnabled();

if (!stocksEnabled) {
  // Тихое игнорирование команд стоков
  return;
}

// Продолжение выполнения функций стоков
```

#### Сценарии использования

##### Временное отключение для обслуживания
```bash
# Отключение стоков для технических работ
STOCKS_ENABLED=false

# Перезапуск контейнеров
docker-compose restart backend tg-bot
```

##### Поэтапный запуск функций
```bash
# Запуск только с генерацией изображений
STOCKS_ENABLED=false

# Позже включение стоковых функций
STOCKS_ENABLED=true
```

#### Пользовательский опыт

##### При отключенных стоках
- **Команда /mystocks**: Показывает сообщение о временном отключении
- **Главное меню**: Кнопка "Мои стоки" отсутствует
- **После генерации**: Только кнопки скачивания, без загрузки на стоки
- **Справка /help**: Информация о стоках не отображается

##### При включенных стоках
- **Полная функциональность** стоковых сервисов
- **Все кнопки и команды** доступны
- **Настройка и загрузка** работают в штатном режиме
...
- **AI Upscaling**: Автоматическое улучшение качества изображений с увеличением разрешения в 4 раза
