...
- [Функционал "Получить файлом"](#-функционал-получить-файлом)
...
## 📁 Функционал "Получить файлом"

### Обзор функциональности

**Функционал "Получить файлом"** позволяет пользователям получать изображения в оригинальном качестве без сжатия Telegram через отправку файлом.

#### Ключевые особенности
- **Отправка без сжатия** через `bot.sendDocument()` вместо `bot.sendPhoto()`
- **Оригинальное качество** 4096x4096 пикселей
...
```javascript
// Кнопка "📁 Получить файлом" добавлена в getImageActionsKeyboard()
keyboard.push([{ text: "📁 Получить файлом", callback_data: `download_file_${imageId}` }]);
```

#### Обработка callback query
```javascript
// Обработчик в основном callback_query handler
...
async function handleDownloadFile(callbackQuery, user) {
  // Извлечение imageId из callback_data
  const imageId = data.split('_')[2];
  
  // Проверка кэша изображения
  const imageData = userImageCache.get(telegramUserId);
  
  // Получение потока изображения от Backend API
  const imageStream = await backendApi.getImageStream(imageId, user.id);
  
  // Создание имени файла с метаданными
  const filename = `ai_image_${timestamp}_${promptSnippet}_${imageId}.jpg`;
  
  // Отправка файла без сжатия
...
1. **Генерация изображения** - пользователь получает сжатое изображение с кнопками
2. **Нажатие "📁 Получить файлом"** - запрос на скачивание оригинала
3. **Статус подготовки** - показ сообщения "Подготавливаю файл..."
4. **Получение файла** - отправка через `bot.sendDocument()` без сжатия
5. **Скачивание** - пользователь может сохранить файл в оригинальном качестве

### Преимущества
- **Оригинальное качество** без потери деталей от сжатия Telegram
- **Удобное именование** файлов с датой и фрагментом промпта
- **Информативные подписи** с метаданными изображения
