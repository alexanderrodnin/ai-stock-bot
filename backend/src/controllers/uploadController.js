...
class UploadController {
   ...
   * POST /api/upload/shutterstock
   */
  async uploadToShutterstock(req, res) {
    const {
      imageId,
      userId,
      title,
    ...
    logger.info('Shutterstock upload request', {
      imageId,
      userId,
      title,
      category,
    ...
    try {
      ...
      res.status(200).json({
        success: true,
        data: result,
        message: 'Image uploaded to Shutterstock successfully'
      });

    } catch (error) {
      logger.error('Shutterstock upload failed', {
        error: error.message,
        imageId,
        userId
      });
      ...
      if (error.message.includes('not configured')) {
        return res.status(400).json({
          success: false,
          error: {
            message: 'Shutterstock service is not configured for this user',
            code: 'SERVICE_NOT_CONFIGURED'
          }
        });
      }
      ...
      res.status(500).json({
        success: false,
        error: {
          message: 'Failed to upload to Shutterstock',
          code: 'SHUTTERSTOCK_UPLOAD_FAILED',
          details: error.message
        }
      });
    }
